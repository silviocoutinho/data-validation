...
module.exports = (app) => {
  function existsOrError(value, msg, code = 400) {        
    ...
    if (Array.isArray(value) && value.length === 0) throw new ValidationError(msg, code);
    if (typeof value === 'string' && !value.trim()) throw new ValidationError(msg, code);
  }
  ...
  function validLengthOrError(value, max, min, field) {
    ...
    if (value.length > max)
      throw new ValidationError(`O limite máximo de caracteres é de ${max} para o campo ${field}`);
    else if (value.length < min)
      throw new ValidationError(`O limite mínimo de caracteres é de ${min} para o campo ${field}`);
  }
  ...
  function strengthPassword(passwd) {
    ...
    if (Array.isArray(errorsArray) && errorsArray.length !== 0) throw new ValidationError(errorsArray[0]);
